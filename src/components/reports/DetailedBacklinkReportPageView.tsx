...
export function DetailedBacklinkReportPageView() {
  ...
  const processedData = useMemo(() => {
    ...
    // Process created links from dead_list
    const createdLinksData: LinkData[] = (payload.dead_list || []).map(link => ({
      url: link.url,
      title: link.title || link.url,
      reason: link.reason || 'No response',
      status: link.status || 0,
      confidence: (link.confidence || 0) / 100,
      content_length: link.content_length || 0,
      category: getCategoryFromStatus(link.status || 0, (link.confidence || 0) / 100)
    }));

    // Process created links from created_links.warning_list
    const warningLinksData: LinkData[] = (payload.created_links?.warning_list || []).map(link => ({
      url: link.url,
      title: link.title || link.url,
    ...
    // Process working links (assuming these are from created_links with working status)
    const workingLinksData: LinkData[] = [];
    // We'll need to determine working links from the summary or other data

    // Combine all links
    const allLinks = [...createdLinksData, ...warningLinksData, ...workingLinksData];
    ...
    return {
      ...
      createdLinks: sortedLinks.filter(link => link.category === 'dead'),
      warningLinks: sortedLinks.filter(link => link.category === 'warning'),
      workingLinks: sortedLinks.filter(link => link.category === 'working')
    };
  ...
  const { report, payload, summary, allLinks, createdLinks, warningLinks, workingLinks } = processedData;
