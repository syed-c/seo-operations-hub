...
// Sample data based on the JSON payload you provided
...
const getCategoryFromStatus = (status: number | null, confidence: number): LinkCategory => {
  ...
  if (status === null || status >= 400 || status === 0) return 'dead';
...
export function TestDetailedBacklinkReport() {
  ...
  const processedData = sampleReportData.map(report => {
    // Process created links warning list
    const warningLinks: LinkData[] = (report.created_links.warning_list || []).map(link => ({
      url: link.url,
    ...
    // Process dead list
    const deadLinks: LinkData[] = (report.dead_list || []).map(link => ({
      url: link.url,
      title: link.title || link.url,
      reason: link.reason || 'No response',
      status: link.status,
    ...
    // Create working links (assuming remaining links are working)
    const workingLinksCount = report.created_links.working || 0;
    const workingLinks: LinkData[] = Array(workingLinksCount).fill(null).map((_, index) => ({
      url: `https://example.com/working-link-${index}`,
      title: `Working Link ${index + 1}`,
      reason: 'Working link',
      status: 200,
      confidence: 90,
      content_length: 1000,
      category: 'working' as LinkCategory
    }));

    // Combine all links
    const allLinks = [...warningLinks, ...deadLinks, ...workingLinks];
  ...
  const allLinks = processedData.flatMap(data => data.allLinks);
  ...
  const exportData = () => {
    const dataToExport = {
      ...
      summary: {
        ...
        working: linksByCategory.working.length,
        dead: linksByCategory.dead.length,
        warning: linksByCategory.warning.length
      }
    };
    ...
    a.href = url;
    a.download = `backlink-report-test.json`;
  ...
  const LinkTable = ({ links }: { links: LinkData[] }) => {
    if (links.length === 0) {
      return (
        <Card>
  ...
  const overallSummary = {
    ...
    total_working: linksByCategory.working.length,
    total_dead: linksByCategory.dead.length,
    total_warning: linksByCategory.warning.length,
    health_percentage: allLinks.length > 0 
      ? Math.round((linksByCategory.working.length / allLinks.length) * 100)
      : 0
  };
  ...
  return (
    <div className="container mx-auto py-6">
      <div className="mb-6">
        <h1 className="text-3xl font-bold">Test Detailed Backlink Report</h1>
        <p className="text-muted-foreground mt-2">
          Testing the detailed backlink report functionality with sample data
        </p>
      </div>
        ...
        <Card>
          <CardContent className="p-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="text-2xl font-bold">{overallSummary.health_percentage}%</p>
                <p className="text-sm text-muted-foreground">Health Score</p>
              </div>
            </div>
            <Progress value={overallSummary.health_percentage} className="mt-3" />
          </CardContent>
        </Card>
        ...
        <Card>
          <CardContent className="p-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="text-2xl font-bold">{overallSummary.total_links_checked}</p>
        ...
        <Card>
          <CardContent className="p-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="text-2xl font-bold text-green-600">{overallSummary.total_working}</p>
        ...
        <Card>
          <CardContent className="p-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="text-2xl font-bold text-red-600">{overallSummary.total_dead}</p>
      ...
      <Tabs defaultValue="all" className="w-full">
        ...
        <TabsContent value="all" className="mt-0">
          <LinkTable links={sortedLinks} />
        </TabsContent>

        {/* Working Links Tab */}
        <TabsContent value="working" className="mt-0">
          <LinkTable links={linksByCategory.working} />
        </TabsContent>

        {/* Dead Links Tab */}
        <TabsContent value="dead" className="mt-0">
          <LinkTable links={linksByCategory.dead} />
        </TabsContent>

        {/* Warning Links Tab */}
        <TabsContent value="warning" className="mt-0">
          <LinkTable links={linksByCategory.warning} />
        </TabsContent>
      </Tabs>
    </div>
  );
