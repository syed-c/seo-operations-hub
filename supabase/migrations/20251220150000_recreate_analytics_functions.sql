-- Recreate analytics functions with proper signatures
-- Drop existing functions first

DROP FUNCTION IF EXISTS get_analytics_summary(uuid, date, date);
DROP FUNCTION IF EXISTS get_top_pages(uuid, date, date);
DROP FUNCTION IF EXISTS get_other_pages_aggregate(uuid, date, date);
DROP FUNCTION IF EXISTS get_all_pages(uuid, date, date, text);
...
SELECT
  ...
  END AS avg_position
FROM gsc_metrics
WHERE gsc_metrics.project_id = p_project_id
AND date BETWEEN p_start_date AND p_end_date
